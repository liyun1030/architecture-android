...
import com.bytedance.core.architecture.base.BaseFragment
import com.bytedance.core.architecture.base.BaseViewModel
import com.bytedance.core.architecture.interfaces.FragmentIsLazyInit
...
import com.bytedance.core.architecture.util.getFragmentIsLazyInit
 ...
 */
abstract class BaseViewsFragment<Binding : ViewBinding, UiState : Any, ViewModel : BaseViewModel<UiState>> :
    BaseFragment(), FragmentIsLazyInit, ViewBindingFragment<Binding, UiState>,
    ViewModelOwner<UiState, ViewModel>, DefaultStateViewManager, MessageCollectorOwner {
    private var _binding: Binding? = null
    private val rootViewGroup by lazy { FrameLayout(requireContext()) }
    private var isLazyInitFirst = true

    // 是否延时初始化，默认为false。
    override val isLazyInit by lazy { getFragmentIsLazyInit() }
...
//        reflectInflateViewBinding(inflater, container)
    ...
    override fun onResume() {
        super.onResume()
        if (isLazyInit && isLazyInitFirst) {
            // 初始化-所有
            // -说明：Lazy效果，通过后观察ViewModel数据来实现。
            initAll()
            isLazyInitFirst = false
        }
    }
